...
function getItemCategoryDetails (stashData, tftPrices) {
  ...
  const heistCurrency = []
  const compassesCompasses = []
  for (const item of stashData.items) {
    if (item.baseType.includes('Charged Compass')) {
      if (verifyUses(item)) {
      ...
      if (verifyWingsNotRevealed(item)) {
        ...
        const itemIlvl = item.ilvl
        const itemPrice = getItemPrice(tftPrices, itemName)
        const itemWings = getBlueprintWings(item)
        console.log(`---Item Details---\nName: ${itemName}\nPrice: ${itemPrice}\nIlvl: ${itemIlvl}\nWings: ${itemWings}\n`)
...
function changeBlueprintName (item, tftPrices) {
  for (const category in tftPrices) {
    for (const tftItem of tftPrices[category].data) {
...
function changeCompassName (item) {
  for (const compassName in compassNames) {
...
// Gets the amount of wings in the blueprint.
function getBlueprintWings (item) {
  const blueprintWingAmount = parseInt(item.properties[2].values[0][0].match(/(?<=\/)(\d)/)[0])
  return blueprintWingAmount
}
...
function verifyWingsNotRevealed (item) {
  ...
  const maxWingsRevealed = item.properties[2].values[0][0].match(/(?<=\/)(\d)/)[0]
